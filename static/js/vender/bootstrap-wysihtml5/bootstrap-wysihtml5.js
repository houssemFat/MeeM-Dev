! function($, wysi) {"use strict";
    ...
    var tpl = {
        ...
        "html" : function(locale, options) {
            var size = (options && options.size) ? ' btn-' + options.size : '';
            return "<li>" + "<div class='btn-group'>" + "<a class='btn" + size + "' data-wysihtml5-action='change_view' title='" + locale.html.edit + "' tabindex='-1'><i class='fa fa-pencil'></i></a>" + "</div>" + "</li>";
        },
    ...
    Wysihtml5.prototype = {
        ...
        initInsertImage : function(toolbar, options) {
            var btswsyieditor = self  = this;
            var caretBookmark;
            var insertImageModal;
            var handleClick = null;
            var insertImage = function(url) {
                scope.editor.currentView.element.focus();
                if (caretBookmark) {
                    scope.editor.composer.selection.setBookmark(caretBookmark);
                    caretBookmark = null;
                }
            ...
            // #FIXME
            if (options.events) {
                if (options.events['insertImage']) { {
                        var self = options.scope || self;
                        handleClick = function() {
                            var editor = btswsyieditor.getEditor() ;
                            options.events['insertImage'].call(self, editor,  function(options) {
                                editor.composer.commands.exec("insertImage", options);
                            });
                        };
                ...
                var self = this;
                insertImageModal = toolbar.find('.bootstrap-wysihtml5-insert-image-modal');
            ...
            toolbar.find('a[data-wysihtml5-command=insertImage]').click(function() {
                ...
                if (!activeButton) {
                    ...
                    caretBookmark = editor.composer.selection.getBookmark();
                    handleClick();
                    return false;
                } else {
                    return true;
